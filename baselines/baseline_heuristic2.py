...
def normalize_kind(raw_kind: str) -> str:
    """Map parser kinds to simpler kinds used by the baseline / gold."""
    # func_entry -> entry
    if raw_kind == "func_entry":
        return "entry"
    
    # before_return / return_stmt -> return
    if raw_kind in ("before_return", "return_stmt"):
        return "return"
    
    # exception handler -> exception
    if raw_kind in ("except", "exception_handler_entry"):
        return "exception"

    # Others, we leave as-is
    return raw_kind
...
def choose_log_level(kind: str, severity_hint: str | None = None) -> str:
    ...
    # If the parser has a severity_hint (DEBUG/INFO/WARNING/ERROR), it takes precedence
